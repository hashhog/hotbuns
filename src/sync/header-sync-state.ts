...
/**
 * Default parameters matching Bitcoin Core mainnet (generated by
 * headerssync-params.py, kernel/chainparams.cpp:193-196).
 *
 * Previously used commitmentPeriod=600 / redownloadBufferSize=12_000 which
 * are both wrong:
 *   - Core mainnet: commitment_period=641, redownload_buffer_size=15218
 *   - Under-sized buffer weakens the DoS protection (fewer commitments
 *     verified before headers are released to the block index).
 */
export const DEFAULT_HEADERS_SYNC_PARAMS: HeadersSyncParams = {
  commitmentPeriod: 641,
  redownloadBufferSize: 15_218,
};
...
export class HeadersSyncState {
   ...
   * @param chainStartMTP - Median time past of the chain start block (seconds).
   *   Used to bound maxCommitments: Core measures elapsed time from chainStartMTP
   *   to now, not from Unix epoch 0. Omitting or passing 0 falls back to a
   *   conservative 0 (i.e. "now - 0") which over-estimates but is safe.
   */
  constructor(
    ...
    // Bug fix: use cryptographically secure RNG for commitOffset.
    // Bitcoin Core uses FastRandomContext() (CSRNG); Math.random() is NOT
    // cryptographically secure — a predictable offset lets an attacker craft a
    // chain that lands no headers at commitment points and bypasses all checks.
    // Reference: headerssync.cpp:23 — FastRandomContext().randrange(period)
    const offsetByte = randomBytes(4).readUInt32BE(0);
    this.commitOffset = offsetByte % syncParams.commitmentPeriod;

    // Bug fix: use cryptographically secure RNG for commitSalt.
    // Math.random() per byte is ~7 bits of entropy per call, giving a salt with
    // far less than 256 bits of entropy and making commitment values predictable.
    this.commitSalt = randomBytes(32);

    // Bug fix: maxCommitments must be bounded by time elapsed since chain start
    // (not since Unix epoch 0). Bitcoin Core computes:
    //   max_seconds = (now - chain_start.GetMedianTimePast()) + MAX_FUTURE_BLOCK_TIME
    //   max_commitments = 6 * max_seconds / commitment_period
    //
    // The old code used `(Date.now()/1000 + 7200)` as max_seconds, which is the
    // absolute Unix timestamp of "now + 2h" — roughly 1.75 billion seconds.
    // For a chain starting at height ~900k (MTP ~2024), the correct elapsed time
    // is ~15 years ≈ 473M seconds, so the old bound was ~3.7x too large,
    // allowing a peer to force ~3.7x more memory allocation before the guard fires.
    //
    // Reference: headerssync.cpp:41-43
    //   max_seconds_since_start = Ticks(NodeClock::now() - NodeSeconds{chain_start.GetMedianTimePast()})
    //                             + MAX_FUTURE_BLOCK_TIME
    //   m_max_commitments = 6 * max_seconds_since_start / commitment_period
    const MAX_FUTURE_BLOCK_TIME = 7200; // seconds (Core's consensus.h)
    const nowSeconds = Math.floor(Date.now() / 1000);
    // chainStartMTP defaults to 0 if not provided (conservative: overestimates
    // the window but is safe; callers should pass the real MTP where available).
    const startSeconds = chainStartMTP ?? 0;
    const maxSecondsSinceStart = Math.max(0, nowSeconds - startSeconds) + MAX_FUTURE_BLOCK_TIME;
    this.maxCommitments = Math.ceil(6 * maxSecondsSinceStart / syncParams.commitmentPeriod);
  ...
  /**
   * Get the maximum commitments bound (used by tests to verify correctness).
   */
  getMaxCommitments(): number {
    return this.maxCommitments;
  }

  /**
   * Get the commit offset (used by tests to verify CSRNG was used).
   * The value is in [0, commitmentPeriod).
   */
  getCommitOffset(): number {
  ...
  private validateAndStoreHeadersCommitments(headers: BlockHeader[]): boolean {
    ...
    // lastHeaderHash is always non-null after construction (initialised to
    // chainStartHash), matching Core's unconditional check:
    //   if (headers[0].hashPrevBlock != m_last_header_received.GetHash()) ...
    // Reference: headerssync.cpp:148-155
    if (!headers[0].prevBlock.equals(this.lastHeaderHash!)) {
